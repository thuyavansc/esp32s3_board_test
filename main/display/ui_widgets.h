...
// Bottom 5-tab nav bar (DASH|GPS|TRIP|SET|TEST), added to every top-level screen.
void ui_add_nav_bar(lv_obj_t *scr, ui_screen_t active);

// Compact drill-down header: "<" back button (left) + title label.
// Used by every screen reached FROM the Test Menu instead of the main nav bar.

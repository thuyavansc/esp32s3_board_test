...
static lv_obj_t *s_eye_btn   = NULL;
...
// Eye toggle (doc 179 Phase 2/D6) — flips the mask, does NOT touch the
// typed text or close anything. Symbol swaps to reflect the new state.
static void _eye_toggle_event(lv_event_t *e) {
    (void)e;
    if (!s_ta || !s_eye_lbl) return;
    bool now_masked = !lv_textarea_get_password_mode(s_ta);
    lv_textarea_set_password_mode(s_ta, now_masked);
    lv_label_set_text(s_eye_lbl, now_masked ? LV_SYMBOL_EYE_CLOSE : LV_SYMBOL_EYE_OPEN);
}
...
void text_keypad_show(lv_obj_t *screen, const char *title, const char *initial,
    ...
    // ── Text area (narrower when password_mode leaves room for the eye
    // toggle on the right, doc 179 Phase 2/D6) ──
    lv_obj_t *ta = lv_textarea_create(panel);
    lv_obj_set_size(ta, password_mode ? 252 : 286, 36);
    ...
    if (password_mode) {
        s_eye_btn = lv_btn_create(panel);
        lv_obj_set_size(s_eye_btn, 28, 36);
        lv_obj_align(s_eye_btn, LV_ALIGN_TOP_RIGHT, 0, 30);
        lv_obj_set_style_bg_color(s_eye_btn, C_BG2, 0);
        lv_obj_set_style_shadow_width(s_eye_btn, 0, 0);
        lv_obj_set_style_border_color(s_eye_btn, C_DIVIDER, 0);
        lv_obj_set_style_border_width(s_eye_btn, 1, 0);
        lv_obj_set_style_radius(s_eye_btn, 6, 0);
        s_eye_lbl = ui_label(s_eye_btn, LV_SYMBOL_EYE_CLOSE, C_TEXT2);
        lv_obj_center(s_eye_lbl);
        lv_obj_add_event_cb(s_eye_btn, _eye_toggle_event, LV_EVENT_CLICKED, NULL);
    }

...
static bool _run_region_test(ram_region_t *r) {
    ...
    ESP_LOGI(TAG, "── %s test: requesting %u bytes ──", r->label, (unsigned)r->test_size);
    ESP_LOGI(TAG, "  Before: free=%u bytes | largest free block=%u bytes",
             (unsigned)free_before, (unsigned)largest_before);

    uint8_t *buf = (uint8_t *)heap_caps_malloc(r->test_size, r->caps);
    if (!buf) {
        ESP_LOGE(TAG, "  FAIL — heap_caps_malloc(%u, caps=0x%x) returned NULL",
                 (unsigned)r->test_size, (unsigned)r->caps);
        r->last_pass = false;
        r->last_used_bytes = 0;
        return false;
    }
    ...
    if (ok) {
        ESP_LOGI(TAG, "  Integrity: PASS — %u bytes written and read back correctly",
                 (unsigned)r->test_size);
    } else {
        ESP_LOGE(TAG, "  Integrity: FAIL — first mismatch at offset %u",
                 (unsigned)mismatch_offset);
    }
    ESP_LOGI(TAG, "  During: used_by_test=%u bytes | free_after_alloc=%u bytes", (unsigned)used_by_test, (unsigned)free_after_alloc);
    ESP_LOGI(TAG, "  After free: free=%u bytes | largest free block=%u bytes",
             (unsigned)free_after_free, (unsigned)largest_after_free);
...
static void _log_both_pools(const char *label) {
    ...
    ESP_LOGI(TAG, "  SRAM  (MALLOC_CAP_INTERNAL) total=%u free=%u min-free=%u largest-block=%u",
             (unsigned)heap_caps_get_total_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_minimum_free_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
    ESP_LOGI(TAG, "  PSRAM (MALLOC_CAP_SPIRAM)   total=%u free=%u min-free=%u largest-block=%u",
...
static void _show_help(void) {
    ...
    printf("  ram test sram     Allocate/write/verify/free %u bytes of internal SRAM\n", (unsigned)RAM_TEST_SRAM_SIZE_BYTES);
    printf("  ram test psram    Allocate/write/verify/free %u bytes of PSRAM (synthetic pattern)\n", (unsigned)RAM_TEST_PSRAM_SIZE_BYTES);

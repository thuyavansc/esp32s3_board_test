...
static void _reconcile_gps_inactive_frame(time_frame_t *f, double end_lat, double end_lon) {
    ...
    if (directions_client_get_route(start_lat, start_lon, end_lat, end_lon, &distance_m,
...
static void _process_new_fix(const gps_data_t *g, double elapsed_sec) {
    ...
    if (s.has_last_fix) {
        ...
        if (straight_m < FARE_CALC_MIN_GAP_DISTANCE_M) {
            ...
            } else if (directions_client_get_route(s.last_lat, s.last_lon, g->lat, g->lon, &moved_m,
                                                     route_pts, FARE_CALC_MAX_POINTS_PER_FRAME, &route_pt_count)) {
                used_route = true;
            } else {
...
void fare_calc_get_snapshot(fare_calc_snapshot_t *out) {
    ...
    out->distance_fare_cents     = distance_fare;
    out->time_fare_cents          = active_time_fare;
    out->gps_inactive_fare_cents  = inactive_time_fare;
